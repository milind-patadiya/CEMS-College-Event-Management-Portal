...
if ($action === 'delete' && $ann_id) {
    try {
        $d = $conn->prepare("DELETE FROM announcements WHERE announcement_id=? AND posted_by=? AND posted_by_role='admin'");
        $d->bind_param("ii", $ann_id, $admin_id); $d->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            if ($edit_id) {
                $s = $conn->prepare("UPDATE announcements SET title=?,content=?,event_id=? WHERE announcement_id=? AND posted_by=? AND posted_by_role='admin'");
                $s->bind_param("ssiii", $title, $content, $event_id, $edit_id, $admin_id);
                $s->execute();
                $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Announcement updated.'];
            } else {
                $s = $conn->prepare("INSERT INTO announcements (title,content,event_id,posted_by,posted_by_role) VALUES (?,?,?,?,'admin')");
                $s->bind_param("ssii", $title, $content, $event_id, $admin_id);
                $s->execute();
                $_SESSION['flash'] = ['type' => 'success', 'msg' => 'Announcement posted. Students and faculty can see it immediately.'];
                // Email all students
                $admin_name_mail = $_SESSION['admin_name'] ?? 'Admin';
                $ev_name_mail = null;
                if ($event_id) {
                ...
                notifyStudentsAnnouncement($conn, $title, $content, $admin_name_mail, 'admin', $ev_name_mail);
            }
            header("Location: announcements.php"); exit();
        } catch (Exception $e) {
...
if ($action === 'edit' && $ann_id) {
    $q = $conn->prepare("SELECT * FROM announcements WHERE announcement_id=? AND posted_by=? AND posted_by_role='admin'");
    $q->bind_param("ii", $ann_id, $admin_id); $q->execute();
...
if ($action === 'list') {
    $q = $conn->prepare("
        ...
        WHERE a.posted_by = ? AND a.posted_by_role = 'admin'
        ORDER BY a.created_at DESC
    ");
    $q->bind_param("i", $admin_id); $q->execute();
    $announcements = $q->get_result()->fetch_all(MYSQLI_ASSOC);
}

$all_events = $conn->query("SELECT event_id, event_name FROM events WHERE status='Approved' ORDER BY event_date DESC")->fetch_all(MYSQLI_ASSOC);
...
<head>
    ...
    <title>Announcements — Admin | CEMS</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
    <link rel="stylesheet" href="../assets/css/style.css">
    <style>
        .ann-card { background:white; border:1px solid var(--border); border-radius:var(--radius-md); overflow:hidden; margin-bottom:14px; box-shadow:var(--shadow-sm); transition:var(--transition); }
        .ann-card:hover { box-shadow:var(--shadow-md); }
        .ann-left-bar { width:4px; flex-shrink:0; }
...
<div class="page-header">
    <div>
        <h1><i class="fas fa-bullhorn me-2" style="color:var(--accent);font-size:1rem;"></i>
            <?= $is_edit ? 'Edit Announcement' : 'New Admin Announcement' ?></h1>
        <p><?= $is_edit ? 'Update the announcement.' : 'Post an announcement as Admin/HOD — visible to all students immediately.' ?></p>
    </div>
...
<div class="form-card">
    <form method="POST">
        <?php if ($is_edit): ?>
        <input type="hidden" name="edit_id" value="<?= $an['announcement_id'] ?>">
        <?php endif; ?>
        <div class="row g-3">
            <div class="col-12">
                <label class="form-label-custom">Title *</label>
                <input type="text" name="title" class="form-control-custom" maxlength="150" required
                       placeholder="e.g. Important Notice Regarding Examinations"
                       value="<?= htmlspecialchars($an['title'] ?? $_POST['title'] ?? '') ?>">
            </div>
            <div class="col-12">
                <label class="form-label-custom">Content *</label>
                <textarea name="content" class="form-control-custom" rows="6" required
                          placeholder="Write the full announcement message..."><?= htmlspecialchars($an['content'] ?? $_POST['content'] ?? '') ?></textarea>
            </div>
            <div class="col-md-6">
                ...
                <select name="event_id" class="form-control-custom">
                    ...
                    <option value="<?= $ev['event_id'] ?>" <?= ($an['event_id'] ?? '') == $ev['event_id'] ? 'selected' : '' ?>>
        ...
        <div class="d-flex gap-2 mt-4">
            <button type="submit" class="btn-primary-custom">
                <i class="fas fa-paper-plane"></i> <?= $is_edit ? 'Update' : 'Post Announcement' ?>
            </button>
...
<div class="page-header">
    <div>
        ...
        <p>Announcements posted as Admin/HOD. Visible to all students.</p>
    </div>
...
<div class="empty-state">
    ...
    <h5>No admin announcements yet</h5>
    <p>Post an announcement to inform all students.</p>
</div>
<?php else: ?>
<?php foreach ($announcements as $ann): ?>
<div class="ann-card">
    <div style="display:flex;">
        <div class="ann-left-bar bar-blue"></div>
        <div class="ann-body">
            ...
            <div class="ann-meta">
                ...
                <span><i class="fas fa-link me-1"></i><?= htmlspecialchars($ann['event_name']) ?></span>
                <?php else: ?>
                <span><i class="fas fa-globe me-1"></i>General</span>
                <?php endif; ?>
                <span style="color:#880e4f;font-weight:600;"><i class="fas fa-user-shield me-1"></i>Admin</span>
            </div>
        </div>
    </div>
